...
export const ShowNote = ({ note }: Props) => {
  ...
  const handleUnarchive = () => {
    startTransition(async () => {
      await unarchiveNote(noteId, user);
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="text-preset-5 h-full text-neutral-950">
                    <FormControl>
                      <Textarea
                        className="text-preset-5 h-full resize-none border-0 whitespace-pre-line text-neutral-800 shadow-none dark:text-neutral-200"
                        disabled={isPending}
